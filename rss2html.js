async function rss2html(url, max) {
  ...
  const items = Array.from(doc.querySelectorAll("channel > item, feed > entry"))
    ...
    .map((item) => ({
      ...
      description: item.querySelector("description") && item.querySelector("description").length > 150 ? item.querySelector("description").textContent.replace(/(<([^>]+)>)/gi, "").substring(0,150)+"..." : item.querySelector("description").textContent,
    }));
